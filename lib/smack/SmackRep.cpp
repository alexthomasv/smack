...
const Expr *SmackRep::load(const llvm::LoadInst &I) {
...
const Expr *SmackRep::load(unsigned R, const llvm::Value *P, const Type *T) {
  bool bytewise = regions->get(R).bytewiseAccess();
...
const Stmt *SmackRep::store(const StoreInst &I) {
  const Value *P = I.getPointerOperand();
  const Value *V = I.getValueOperand()->stripPointerCastsAndAliases();
  assert(P->getType()->isPointerTy() && "Expected pointer type.");
  assert(!V->getType()->isAggregateType() && "Unexpected store value.");
  return store(regions->idx(I), V->getType(), expr(P), expr(V));
}
...
std::list<ProcDecl *> SmackRep::procedure(llvm::Function *F) {
  ...
  if (callers.empty() || !F->isVarArg())
    procs.push_back(procedure(F, nullptr));
